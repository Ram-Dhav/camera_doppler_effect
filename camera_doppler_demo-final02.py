"""
Doppler Effect Demonstration - Raspberry Pi + USB Webcam + Speaker
------------------------------------------------------------------------------
Detects hand distance continuously via webcam and maps it to audio pitch
and beep rate, simulating the Doppler effect. Works across different
people's skin tones via per-session calibration (motion-based hand
detection, not a fixed color guess).
...
FEATURES:
  ...
  - Smooth audio-only pacing (no LED needed)
...
Requires: opencv (cv2), numpy, matplotlib
"""
...
# ---------------------------------------------------------------------
# BEEP / TONE / SMOOTHING CONFIGURATION
# ---------------------------------------------------------------------
NEAR_BEEP_INTERVAL = 0.08
FAR_BEEP_INTERVAL = 0.6
NEAR_FREQ_HZ = 1000
FAR_FREQ_HZ = 300
TONE_DURATION = 0.12
NUM_TONE_STEPS = 16
...
def print_startup_banner():
    ...
    print("  change the audio pitch and beep rate.")
    print("")
...
def offer_cached_calibration():
    ...
    try:
        new_lower = np.array(cached["ycrcb_lower"], dtype=np.uint8)
        new_upper = np.array(cached["ycrcb_upper"], dtype=np.uint8)
        new_min_area = float(cached["min_area"])
        new_max_area = float(cached["max_area"])
    except (KeyError, ValueError, TypeError) as e:
        print("Saved calibration file looks incomplete or corrupted ({}).".format(e))
        print("Recalibrating fresh instead.")
        return None

    current_ycrcb_lower = new_lower
    current_ycrcb_upper = new_upper
    print("Loaded saved calibration.")
    print("")
    return new_min_area, new_max_area
...
def attempt_calibration(cap, roi_coords):
    ...
    if not background_frames:
        return (FALLBACK_MIN_HAND_AREA * resolution_scale_factor,
                FALLBACK_MAX_HAND_AREA * resolution_scale_factor, False)
...
def step_to_beep_interval(step):
    fraction = step / float(NUM_TONE_STEPS - 1)
    return FAR_BEEP_INTERVAL - fraction * (FAR_BEEP_INTERVAL - NEAR_BEEP_INTERVAL)
...
def build_session_summary_lines(timestamps, areas, intervals, distance_fractions, min_area, max_area):
    ...
    lines.append("  Beep interval range  : {:.3f}s (fastest) - {:.3f}s (slowest)".format(
...
def build_physics_explanation_lines(session_real_shifts):
    ...
    lines.append("   produces a shift of a tiny fraction of a Hz. The audio tone")
    lines.append("   in this demo is DELIBERATELY EXAGGERATED across a much wider")
    lines.append("   range so the underlying concept - frequency changing with")
    lines.append("   relative motion - is actually perceivable.)")
    lines.append("+" + "-" * 62 + "+")
    return lines
...
def save_session_report(summary_lines, physics_lines, session_start_str):
    timestamp_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = os.path.join(GRAPH_OUTPUT_DIR, "doppler_session_{}.txt".format(timestamp_str))
    try:
        with open(filename, "w") as f:
            f.write("Session recorded: {}\n".format(session_start_str))
            f.write("=" * 50 + "\n\n")
            f.write("\n".join(summary_lines))
            f.write("\n\n")
            f.write("\n".join(physics_lines))
...
def export_session_csv(timestamps, areas, distance_fractions, intervals, demo_freqs, real_shifts, session_start_str):
    if len(timestamps) < 2:
    ...
    try:
        with open(filename, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Session recorded:", session_start_str])
            writer.writerow([])
            writer.writerow(["time_sec", "hand_area_px", "distance_fraction",
                              "beep_interval_sec", "demo_tone_hz", "real_doppler_shift_hz"])
...
def generate_session_graph(timestamps, areas, intervals):
    ...
    ax2.set_ylabel("Beep interval (seconds)")
...
def run_demo():
    ...
    global resolution_scale_factor
    resolution_scale_factor = (actual_width * actual_height) / float(REQUESTED_WIDTH * REQUESTED_HEIGHT)
    if abs(resolution_scale_factor - 1.0) > 0.05:
        print("Note: camera resolution differs from the {}x{} baseline area constants".format(
            REQUESTED_WIDTH, REQUESTED_HEIGHT))
        print("were tuned for. Scaling fallback thresholds by {:.2f}x accordingly.".format(
    ...
    try:
        while True:
            ...
            interval = step_to_beep_interval(step)
        ...
        if session_timestamps:
            session_start_str = datetime.datetime.fromtimestamp(
                session_timestamps[0]).strftime("%d %B %Y, %H:%M:%S")
        ...
        if summary_lines and physics_lines:
            save_session_report(summary_lines, physics_lines, session_start_str)
